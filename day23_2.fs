...
// let fileName = "/Users/jan/Downloads/input23"
...
let rec backtrackGoal (edgesByDest:Map<Pos,Edge list>) (goal:Pos) (steps:int) : Pos*int =
    let prev = edgesByDest[goal]
    if prev.Length > 1 then
        goal,steps
    else
        let (f,_,c) = prev.Head
        let steps = steps + c
        let goal = f
        backtrackGoal edgesByDest goal steps 
...
let solve (edges:Edge list) =
    ...
    let goal,finalsteps = backtrackGoal edgesByDest goal 0
    
    printfn $"GOAL = {goal} finalsteps = {finalsteps}"

    let rec solve (goal:Pos) (curr:Pos) (visited:Set<Pos>) (steps:int) : int =
...
//                    printfn $"Dead end: {curr}"
                    ...
                    let followEdge ((f,t,c):Edge) : int =
                        let steps = steps + c
                        let curr = t
                        solve goal curr visited steps
                    edges |> List.map followEdge |> List.max
    solve goal start Set.empty finalsteps
